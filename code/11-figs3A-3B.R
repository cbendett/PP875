...
pdf("../figures/11-Fig3AB.pdf") # set the graphics device for all plots

setwd("../results/HyDe/10Mb-concatenation-ch3") #replace PathTo with the correct path and run if not in the correct folder
...
ggplot(significant_mutica_boeoticum, aes(x = Hybrid, y = Gamma, fill = Hybrid)) +
  geom_violin(
    trim = TRUE,
    scale = "width",
    color = NA
